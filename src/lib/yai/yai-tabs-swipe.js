...
class YaiTabsSwipe {
    ...
    constructor(customConfig = {}) {
        const swipeConfig = {
            ...
            boundaryBehavior: {
                circular: true,        // Loop from last to first tab and vice versa
        ...
        this._boundSuppressClickHandler = (event) => {
            const suppressedLink = this.swipeState.suppressClickLink;
            if (!suppressedLink) return;
            const selector = typeof this.config.cancelDragClickSelector === 'string'
                ? this.config.cancelDragClickSelector.trim()
                : '';
            if (!selector) return;

            const clickedLink = event.target?.closest?.(selector);
            if (clickedLink !== suppressedLink) return;
            ...
            this.swipeState.suppressClickLink = null;
        };

        this._boundContextMenuReset = () => {
            if (!this.isDragging()) return;
            this.resetDraggingState();
        };

        this._boundWindowBlurReset = () => {
            if (!this.isDragging()) return;
            this.resetDraggingState();
        };
    }
    ...
    getTabsContainer(target) {
        const container = target.closest('[data-yai-tabs][data-swipe], [data-yai-tabs][data-swipe-pages]');
    ...
    shouldIgnoreSwype(event) {
        ...
        const allowedInteractive = target.closest?.('[data-swipe-allow]');
    ...
    ensureParentChainVisible(container) {
        ...
        while (current) {
            const parent = current.parentElement?.closest('[data-yai-tabs][data-swipe], [data-yai-tabs][data-swipe-pages]');
            if (parent) {
    ...
    handleBoundaryBehavior(container, offset, direction, originContainer = null) {
        ...
        const parentTabContainer = container.parentElement?.closest('[data-yai-tabs][data-swipe], [data-yai-tabs][data-swipe-pages]');
    ...
    mouseIsDown(moveType, event, target) {
        ...
        this.swipeState.activePanel = panel;
        ...
        const clickCancelSelector = typeof this.config.cancelDragClickSelector === 'string'
            ? this.config.cancelDragClickSelector.trim()
            : '';
        this.swipeState.dragOriginLink = moveType === 'mouse' && clickCancelSelector
            ? target.closest?.(clickCancelSelector) ?? null
            : null;
        this.swipeState.shouldSuppressClick = false;
        ...
        // Execute swipeStart hook — returning false cancels the swipe
        const swipeStartResult = this._executeHook('swipeStart', {
            panel,
            moveType,
            startX,
            startY,
        ...
        if (swipeStartResult === false) {
            this.swipeState.isDragging = false;
            this.swipeState.activePanel = null;
    ...
    mouseIsMoving(moveType, event, target) {
        const panel = this.swipeState.activePanel ?? this.getTabsPanel(target);
        if (!panel) return;
        ...
        if (this.swipeState.isDragging) {
            ...
            if (
                moveType === 'mouse'
                && this.swipeState.dragOriginLink
                && !this.swipeState.shouldSuppressClick
            ) {
                const dragThreshold = this.config.clickCancelThreshold || 8;
                if (Math.abs(deltaX) > dragThreshold || Math.abs(deltaY) > dragThreshold) {
                    this.swipeState.shouldSuppressClick = true;
                }
            }
    ...
    mouseGoesUp(moveType, event, target) {
        ...
        if (this.swipeState.isDragging) {
            ...
            if (moveType === 'mouse' && this.swipeState.shouldSuppressClick && this.swipeState.dragOriginLink) {
                this.swipeState.suppressClickLink = this.swipeState.dragOriginLink;
                if (this._suppressClickTimeout) {
                    clearTimeout(this._suppressClickTimeout);
                }
            ...
            this.swipeState.dragOriginLink = null;
            this.swipeState.shouldSuppressClick = false;
    ...
    watchHooks() {
        ...
        document.addEventListener('click', this._boundSuppressClickHandler, true);
        document.addEventListener('contextmenu', this._boundContextMenuReset, true);
        window.addEventListener('blur', this._boundWindowBlurReset);
    ...
    resetDraggingState() {
        ...
        const allTabPanels = document.querySelectorAll('[data-swipe] [data-tab], [data-swipe-pages] [data-tab]');
    ...
    destroy() {
        document.removeEventListener('click', this._boundSuppressClickHandler, true);
        document.removeEventListener('contextmenu', this._boundContextMenuReset, true);
        window.removeEventListener('blur', this._boundWindowBlurReset);
        this.swipeState.suppressClickLink = null;
        if (this._suppressClickTimeout) {
            clearTimeout(this._suppressClickTimeout);
            this._suppressClickTimeout = null;
