  ...
  <Teleport to="body">
    ...
    >
      <div v-if="show" class="fixed inset-0 z-[70] flex items-center justify-center p-4">
        ...
        <!-- Modal Panel -->
        <div class="relative bg-black/80 backdrop-blur-xs border border-white/10 shadow-2xl w-full max-w-lg overflow-hidden">
          <header class="px-3 py-2 border-b border-white/10 flex items-center justify-between bg-black/30">
            <h2 class="text-[11px] font-semibold text-white">{{ title }}</h2>
            <button
              @click="emit('close')"
